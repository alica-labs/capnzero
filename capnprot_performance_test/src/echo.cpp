...
//#define DEBUG_SENDER
int16_t j;
std::vector<capnzero::Capnprotoperformancetest>v;

void callback(::capnp::FlatArrayMessageReader& k) {

    std::cout << "subscriber called for port 5500 and rcvd message ...." << std::endl;
    k.getRoot<capnzero::Capnprotoperformancetest>().toString().flatten().cStr();

    s3=k.getRoot<capnzero::Capnprotoperformancetest>().getString();

    //std::cout << k.getRoot<capnzero::Capnprotoperformancetest>().getNumber()<<std::endl;//
           // .toString().flatten().cStr() << std::endl;
           j=int16_t (k.getRoot<capnzero::Capnprotoperformancetest>().getNumber());
    s0 = k.getRoot<capnzero::Capnprotoperformancetest>().toString().flatten().cStr();
    //=s0[2];
...
//    sub->connect(capnzero::CommType::TCP, "141.51.122.62:5555");
  ...
  //  s3="hi";
    capnzero::Capnprotoperformancetest::Builder lu= s1.initRoot<capnzero::Capnprotoperformancetest>();
