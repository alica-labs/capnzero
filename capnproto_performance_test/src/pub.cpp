        ...
        {     beaconMsgBuilder.setNumber(k);
           ...
           // measuringMap.emplace(msgBuilder, std::chrono::high_resolution_clock::now());
            measuringMap.emplace(k, std::chrono::high_resolution_clock::now());
            {
                ...
                std::cout << "Master is going to publish number : "<<k << std::endl;}
...
//auto j= reader.getOptions<capnzero::Capnprotoperformancetest>().nestingLimit;
    ...
    leo = reader.getRoot<capnproto::Capnprotoperformancetest>().toString().flatten().cStr() ;
    k=reader.getRoot<capnproto::Capnprotoperformancetest>().getNumber();
    Scorpio=reader.getRoot<capnproto::Capnprotoperformancetest>().getString();
    ...
    if (mapEntry != measuringMap.end()) {
        ...
        Mymap.emplace(k,time_passed);

        measuringMap.erase(k);
    }
    else
    {
